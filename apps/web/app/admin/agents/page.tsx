import Link from "next/link";

const RAILWAY_API_URL = process.env.RAILWAY_API_URL;
const ADMIN_SECRET = process.env.ADMIN_SECRET ?? "";

const MODEL_LABELS: Record<string, string> = {
  "claude-opus-4-6": "Opus 4.6",
  "claude-sonnet-4-6": "Sonnet 4.6",
  "claude-haiku-4-5-20251001": "Haiku 4.5",
};

function modelLabel(model: string | null): string {
  if (!model) return "—";
  return MODEL_LABELS[model] ?? model;
}

function timeAgo(iso: string): string {
  const diff = Date.now() - new Date(iso).getTime();
  const m = Math.floor(diff / 60_000);
  if (m < 1) return "just now";
  if (m < 60) return `${m}m ago`;
  const h = Math.floor(m / 60);
  if (h < 24) return `${h}h ago`;
  return `${Math.floor(h / 24)}d ago`;
}

interface AgentMeta {
  name: string;
  role: string;
  settings: { model: string | null; temperature: number | null; max_tokens: number | null };
  last_modified: string;
  size_bytes: number;
}

async function loadAgents(): Promise<AgentMeta[]> {
  if (!RAILWAY_API_URL) return [];
  try {
    const res = await fetch(`${RAILWAY_API_URL}/admin/agents`, {
      headers: { "x-admin-secret": ADMIN_SECRET },
      cache: "no-store",
    });
    if (!res.ok) return [];
    const data = await res.json();
    return data.agents ?? [];
  } catch {
    return [];
  }
}

export default async function AgentsPage() {
  const agents = await loadAgents();

  const roleColors: Record<string, string> = {
    scout: "text-blue-400 bg-blue-400/10 border-blue-400/30",
    zero: "text-purple-400 bg-purple-400/10 border-purple-400/30",
    obi: "text-[#00e87a] bg-[#00e87a]/10 border-[#00e87a]/30",
    sal: "text-yellow-400 bg-yellow-400/10 border-yellow-400/30",
    sully: "text-orange-400 bg-orange-400/10 border-orange-400/30",
  };

  return (
    <div>
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Agent Skills</h1>
          <p className="text-gray-500 text-sm mt-1">
            Edit the MD files that define each agent&apos;s behavior, knowledge, and settings.
            Changes take effect on the next agent invocation — no redeploy needed.
          </p>
        </div>
      </div>

      {!RAILWAY_API_URL && (
        <div className="bg-yellow-400/10 border border-yellow-400/30 rounded-lg px-5 py-4 text-yellow-400 text-sm mb-6">
          RAILWAY_API_URL is not configured. Agent files cannot be loaded.
        </div>
      )}

      <div className="bg-[#111] border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-800 text-xs uppercase tracking-wider text-gray-500">
              <th className="text-left px-6 py-3">Agent</th>
              <th className="text-left px-6 py-3">Role</th>
              <th className="text-left px-6 py-3">Model</th>
              <th className="text-right px-6 py-3">Temp</th>
              <th className="text-right px-6 py-3">Max tokens</th>
              <th className="text-left px-6 py-3">Last saved</th>
              <th className="px-6 py-3" />
            </tr>
          </thead>
          <tbody>
            {agents.map((agent) => (
              <tr
                key={agent.name}
                className="border-b border-gray-900 hover:bg-white/[0.02] transition-colors"
              >
                <td className="px-6 py-4">
                  <span
                    className={`text-xs font-mono font-bold px-2 py-1 rounded border uppercase ${
                      roleColors[agent.name] ?? "text-gray-400 bg-gray-400/10 border-gray-400/30"
                    }`}
                  >
                    {agent.name}
                  </span>
                </td>
                <td className="px-6 py-4 text-sm text-gray-300 max-w-xs truncate">
                  {agent.role || <span className="text-gray-600 italic">No role defined</span>}
                </td>
                <td className="px-6 py-4 text-sm font-mono text-gray-400">
                  {modelLabel(agent.settings.model)}
                </td>
                <td className="px-6 py-4 text-right text-sm font-mono text-gray-400 tabular-nums">
                  {agent.settings.temperature ?? "—"}
                </td>
                <td className="px-6 py-4 text-right text-sm font-mono text-gray-400 tabular-nums">
                  {agent.settings.max_tokens?.toLocaleString() ?? "—"}
                </td>
                <td className="px-6 py-4 text-sm text-gray-500">
                  {timeAgo(agent.last_modified)}
                </td>
                <td className="px-6 py-4 text-right">
                  <Link
                    href={`/admin/agents/${agent.name}`}
                    className="text-xs font-medium text-[#00e87a] hover:text-white border border-[#00e87a]/40 hover:border-white/30 px-3 py-1.5 rounded transition-colors"
                  >
                    Edit
                  </Link>
                </td>
              </tr>
            ))}
            {agents.length === 0 && (
              <tr>
                <td colSpan={7} className="px-6 py-16 text-center text-gray-700">
                  {RAILWAY_API_URL ? "No agents found" : "RAILWAY_API_URL not configured"}
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      <div className="mt-4 flex items-start gap-2 text-xs text-gray-600">
        <span className="mt-0.5">⚠</span>
        <span>
          Agent MD files are loaded as system prompts on every Claude API call. Edits are live
          immediately. Use the version history in the editor to roll back if needed.
        </span>
      </div>
    </div>
  );
}
